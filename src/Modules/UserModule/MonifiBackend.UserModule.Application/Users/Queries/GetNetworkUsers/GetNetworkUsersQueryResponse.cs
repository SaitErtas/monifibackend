...
public class GetNetworkUsersQueryResponse
{
    ...
    {
        var clearMeFirstNetworkUsers = meFirstNetworkUsers.Select(x => new GetUser(x.Email.CapitalizeFirstAndHideText(), x.ReferanceCode.CapitalizeFirstAndHideText(), x.FullName, $"{stringLocalizer["Member"]}", x.Id)).ToList();
        ...
        var clearUser = new GetUser(lead.Email.CapitalizeFirstAndHideText(), lead.ReferanceCode.CapitalizeFirstAndHideText(), lead.FullName.CapitalizeFirstAndHideText(), $"{stringLocalizer["Leader"]}", lead.Id);
