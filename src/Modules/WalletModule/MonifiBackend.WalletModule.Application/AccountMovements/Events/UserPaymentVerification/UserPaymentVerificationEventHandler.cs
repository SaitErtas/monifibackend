    ...
    public async Task Handle(UserPaymentVerificationEvent request, CancellationToken cancellationToken)
    {
        ...
        foreach (var accountMovement in accountMovements)
        {
            ...
            if (accountMovement.Wallet.CryptoNetwork.Id == BSCSCAN_VALUE)
            {
                ...
                var appropriateTransfers = bep20TokenTransferEventsResult.Result.Where(x =>
                x.ContractAddress == BSCSCAN_ADDRESS &&
                x.TokenSymbol == BSCSCAN_TOKEN_SYMBOL &&
                AmountCheck(IntToDec(x.Value, x.TokenDecimal), accountMovement.Amount)).ToList();
                ...
                foreach (var appropriateTransfer in appropriateTransfers)
                {
                    ...
                    if (userMoments.Any(x => x.Hash == appropriateTransfer.Hash))
                    {
            ...
            else if (accountMovement.Wallet.CryptoNetwork.Id == TRONNETWORK_VALUE)
            {
                ...
                x.ContractAddress == TRONNETWORK_ADDRESS &&
            ...
            if (accountMovement.TransactionStatus == TransactionStatus.Successful)
            {
                ...
                if (userAccountMovement.Count == 0)
                {
                    ...
                    var bonus = AccountMovement.CreateNew(bonusAmount, Core.Domain.Base.BaseStatus.Active, TransactionStatus.Successful, ActionType.Bonus, accountMovement.Wallet.UserId, string.Empty, string.Empty, DateTime.Now);
